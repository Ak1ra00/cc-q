# QUASAR on the Coldcard Q: board config.
#
# Derived from the Coldcard Q1 board files by Coinkite (see COPYING-CC). The
# Bitcoin libraries are gone; the only extra C module is the game.
#
MCU_SERIES = l4
...
# GCC 13 flags a false positive inside bundled MicroPython
CFLAGS_EXTRA += -Wno-error=dangling-pointer

# the game engine lives in ../game and is added as a user C module
USER_C_MODULES = boards/$(BOARD)/c-modules

# our python: boot, system screens, firmware reinstall
# - do not want contents of stm32/boards/manifest.py
FROZEN_MANIFEST = boards/$(BOARD)/python/manifest.py

# the game renders every frame, so it gets real optimisation
build-COLDCARD_Q1/quasar/%.o: COPT = -O2 -DNDEBUG
